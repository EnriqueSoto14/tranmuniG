...
//alert(idP);
 ...
 var personalAnteriores = document.getElementById("personalA").innerHTML;
  ...
  function eliminaPersonal(idP) {
    // body...
    var a = confirm('¿Esta seguro de que desea borrar a esta actividad?');
    var idPer= idP;
    var idRo = $('#idRol').val();
    var conta = $('#contSI').val();
    if(a){
        window.location="/eliminaPersonal/" + idPer +"/"+ idRo +"/"+ conta +"";
...
<main role="main" class="">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">
          ...
          </a><label style="font-size: 25px;">&nbsp;MODIFICACIÓN DE ACTIVIDADES DE PERSONAL PARA LA UNIDAD DE <br>&nbsp;{{$nom_dep}}</label>&nbsp;
          <br>
     ...
     <div class="col-md-8 order-md-1">
          ...
          <label style="font-size: 10px; color: blue;">Por favor, seleccione a un responsable para la actividad de la lista de personal y pulse el botón agregar, puede agregar uno o varios responsables a la misma actividad.</label>
           <div id="msjGuardar">
            <table>
              <tr>
          ...
          <form class="needs-validation" action="{{ route('runModificaActividad') }}" name="formulario" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
              ...
              <button id="botagregar" type="button" class="btn btn-primary btn-sm" onclick="agregarPerosnal()" disabled>Agregar</button>
                ...
                <div class="col-md-4 mb-4">
@foreach($objDatosQ8 as $dato)
                <label for="fi">Fecha de Inicio:</label>
                 <input type="date" name="fi" id="fi" class="form-control" placeholder="Fecha de inicio" value="{{$dato['fecha_inicio']}}" required>
              ...
              <div class="col-md-4 mb-4">
                <label for="ft">Fecha de Término:</label>
                 <input type="date" name="ft" id="ft" class="form-control" placeholder="Fecha de término" value="{{$dato['fecha_fin']}}" required>
                <div class="invalid-feedback">
                  Rellenar este campo con texto es requerido.
                </div>
              </div>
...
<input type="hidden" name="contSI" id="contSI" value="{{$conta}}">
              <input type="hidden" id="acti" name="acti" value="{{$actividad}}">
            ...
            <table  id="example" name="example" class="table table-striped table-bordered dataTable no-footer" cellspacing="0" width="100%" aria-describedby="example_info" role="grid" style="width: 100%; ">
              <thead style="text-align: center;" class="thead-dark" id="panel">
                  ...
                  <tr>
                    ...
                    <th style="text-align: center; font-size: 12px;" WIDTH="35%">Nuevos Responsables de la actividad</th>
                    <th style="text-align: center; font-size: 12px;" WIDTH="10%">Accion</th>
              ...
              <tbody id="personalA">
                 <?php $c=0;?>
               @foreach($objDatosQ9 as $item)

                 <tr>
                  <?php $c=$c+1;?>
                <!--  <input type="hidden" id="ids{{$c}}" name="ids{{$c}}" value="{{$item['id_personal']}}"/> -->
                  <td>{{$item['nombre']}}</td>
                  <td align="center"><button type="button" class="btn btn-sm btn-danger" style="font-size: 11px;"  title="Eliminar personal" onclick="eliminaPersonal({{$item['id_personal']}})"><i class="fa fa-trash" aria-hidden="true"></i></button></td>  
...
@foreach($objDatosQ8 as $dato)
            Actividad:
            <textarea class="form-control" id="actividad" name="actividad" rows="10" cols="50" placeholder="Actividad" style="margin-top: 0px; margin-bottom: 0px; height: 131px;">{{$dato['actividad']}}</textarea>
            <hr class="mb-2">
            Recursos:
             <textarea class="form-control" id="recursos" name="recursos" rows="10" cols="50" placeholder="Recursos" style="margin-top: 0px; margin-bottom: 0px; height: 131px;">{{$dato['recursos']}}</textarea>
             <hr class="mb-2">
            Conclusiones:
             <textarea class="ckeditor" id="conclusiones" name="conclusiones" rows="10" cols="50" placeholder="Conclusiones" style="margin-top: 0px; margin-bottom: 0px; height: 131px;">{{$dato['conclusiones']}}</textarea>
    @endforeach
             <hr class="mb-2">
               <h4>Evidencias</h4>
...
<p>  
 ...
 < ?php }?> -->
             ...
             @foreach($objDatosQ8 as $dato)
                ...
                <option value="{{$dato['estatus']}}" selected hidden >{{$dato['estatus']}}</option>
      ...
      @endforeach
           ...
           <!-- <button type="button" class="btn btn-success btn-lg" onclick="ejecutarAccion('S')">Guardar y salir</button>
            ...
            <hr class="mb-2">
                ...
                <button id="guardarbut" type="button" class="btn btn-success btn-lg" onclick="ejecutarAccion()">Modificar</button>
            <a href="{{route('home')}}">
